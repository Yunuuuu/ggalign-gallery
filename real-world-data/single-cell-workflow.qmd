...
<https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE210347>. A detailed
description of the preprocessing workflow can be found here:
...
#| echo: false
#| output: asis
umap_subset <- datasource("umap_subset.rds")
summarized <- datasource("summarized.rds")
umap <- datasource("umap.rds")
clidata <- datasource("clidata.rds")
cat(knitr::knit_child(text = c(
    "```{r}",
    sprintf('umap_subset <- readRDS("%s")', umap_subset),
    sprintf('summarized <- readRDS("%s")', summarized),
    sprintf('umap <- readRDS("%s")', umap),
    sprintf('clidata <- readRDS("%s")', clidata),
    "```",
    ""
), quiet = TRUE))
```
